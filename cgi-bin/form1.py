...
results = "100" #Количество возвразаемых результатов
...
for i in range(0,number_of_places):
    ...
    for item in x:
        if item["name"]=="Кальян-бар" or item["name"]=="Сауна":
            ...
            if "Availabilities" in data["features"][i]["properties"]["CompanyMetaData"]["Hours"] and "Features" in data["features"][i]["properties"]["CompanyMetaData"]:
                goodplace = 1
    if goodplace==1:
        if "Hours" in data["features"][i]["properties"]["CompanyMetaData"]:
            places.append([])
